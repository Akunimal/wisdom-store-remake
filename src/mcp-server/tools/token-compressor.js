...
function detectCommandCategory(commandStr) {
  ...
  if (base === 'jest' || base === 'vitest' || base === 'pytest' || base === 'rspec' ||
      (base === 'npm' && (args[0] === 'test' || (args[0] === 'run' && args[1] === 'test'))) ||
      ((base === 'yarn' || base === 'pnpm') && args[0] === 'test') ||
...
export function compressOutput(command, rawOutput, options = {}) {
  ...
  // Steps 3-4 run only for non-git output. Git diffs are line-oriented and
  // promised "nearly lossless": collapsing 3+ identical lines (e.g. "+}") into
  // "+} [×3]" or reordering grouped lines yields a diff the agent cannot apply.
  // The git filter already self-truncates noise with domain knowledge.
  if (category !== 'git') {
