...
> **`check_symbols` and `get_file_skeleton` accept `format: "json"`** for structured, machine-parseable output.
...
anti-hallucination setup [--project PATH]      # Configure MCP server + hooks
```

Run via `npx anti-hallucination <cmd>`, or `node bin/cli.js <cmd>` from a clone. All commands accept `--project PATH`; `check` and `skeleton` accept `--json`.
...
> **`check_symbols` y `get_file_skeleton` aceptan `format: "json"`** para salida estructurada y parseable por máquina.
